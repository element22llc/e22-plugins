...
| **Path-scoped rules** | `templates/scaffold/claude/rules/steer-*.md` (24 files) | Installed into the consumer repo as `.claude/rules/steer-*.md` by `/steer:init` / `/steer:adopt`, repaired by `/steer:sync` (capability `path-scoped-rules`); each loads when Claude reads a file its `paths:` frontmatter matches | Prose is portable; **delivery is repo-bound**, not hook-bound — a repo that never adopted them never gets them |
