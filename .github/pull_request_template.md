...
changes plugin behavior and needs a changelog fragment; editing this one does not.
...
- [ ] **Plugin behavior** — `plugins/steer/{skills,rules,hooks,templates,scripts,policy}/`, or any of the three version-bearing manifests (incl. the root `.github/plugin/marketplace.json`) → needs a changelog fragment
...
- [ ] Added my own fragment under `.changes/unreleased/` (`mise run changelog:new`, or the YAML file directly for multi-line prose — see AUTHORING.md → "CHANGELOG & versioning")
- [ ] `CHANGELOG.md` **untouched** — it is generated by `changie merge` at release
