...
…while preserving everything outside `steer:managed:start`/`end` verbatim.

| Fixture | What it proves |
|---|---|
| `preserve-human-notes` | Content **outside** the block (human `## Team notes`) is preserved byte-for-byte. |
| `unknown-marker-survival` | An unknown-but-valid marker (`steer:custom=…`) survives the rewrite. |
