...
SCAFFOLD_MSG="Scaffold check: this repo has NO root mise.toml — proceeding to write ${CLASS} (${SAFE_FILE}) leaves it with zero toolchain/CI/PR-template. The bundled scaffold (mise.toml, compose.yaml, CI, PR template, .gitignore) is product-INDEPENDENT — run /steer:init to install it (greenfield path), or at minimum lay down mise.toml + CI; do NOT hand-write package.json / build config / CI from scratch. This scaffold reminder re-fires on each new file you write until a root mise.toml exists."
