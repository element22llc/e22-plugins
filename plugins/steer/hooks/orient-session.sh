...
# needs the topology or the build resumes against the wrong spine. Full topology:
...
if [ -n "${POLY_ROLE}" ]; then
	...
	if [ "${POLY_ROLE}" = "workspace" ]; then
		...
		printf '`/steer:status`, `/steer:audit`, `/steer:spec roadmap`, `/steer:setup protect`) must '
