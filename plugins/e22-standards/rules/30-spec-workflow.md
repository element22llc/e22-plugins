...
- **Starting a user-facing feature** → `/spec/features/[id]/intent.md` +
  ...
  **`/e22-standards:e22-spec`** (or **`/e22-standards:e22-build`** for a PO),
...
workflow (`/e22-standards:e22-spec`) draws on as it authors the spec. If unsure
...
(**`/e22-standards:e22-spec`**); a PO driving it uses **`/e22-standards:e22-build`**. Design exports: read the **local export** via
`/e22-standards:e22-design-sources` — never fetch the URL (it 403s).
