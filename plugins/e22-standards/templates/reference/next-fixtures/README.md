# /e22-next golden fixtures

Prose scenarios that pin the intended **cross-workflow** arbitration of
[`/e22-next`](../../../skills/e22-next/SKILL.md) against the shared
...
exactly what `/e22-next` owns and the per-skill blocks deliberately don't.
...
(the trap the precedence must avoid). Walk the `/e22-next` dimension table plus
