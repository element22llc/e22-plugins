...
delivery of one issue, not backlog management (the backlog skill never edits code).

A passing response has to do **both** of these:

1. **Name the owning skill.** `/steer:work` appears somewhere in the response -
...
- does the **wrong** workflow's work instead - in particular `issues`, which
