...
| `../github/dependabot.yml` | `.github/dependabot.yml` | Dependabot config - `github-actions` ecosystem live; `npm`/`pip`/`docker`/`terraform` blocks commented out for `/steer:init`/`/steer:adopt` to uncomment per detected stack (`terraform` is the `infra` profile's). Groups updates and `ignore`s majors (deferred to a `policy/versions.yml` decision). |
