...
- Asked to "protect main", set up or check **branch protection** / merge rules on a GitHub-adopted repo? → run **`/steer:protect`** — reads `policy/branch-protection.yml`, diffs it against the repo's live GitHub settings, and (only on your confirmation) applies what's missing via `gh api`. steer is advisory locally (you are not the gate); this configures the *real* server-side gate. Verify-only by default.
