...
| `/steer:questions` | `/steer:spec`, `/steer:issues` | Sweep every open question across the spine (each feature's `intent.md` + `vision.md`) and walk the PO/dev through answering each one, folding the decision back into the spec — promoting a question to a tracked issue is one sub-step. |
