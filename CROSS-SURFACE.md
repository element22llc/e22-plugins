...
|---|---|---|---|
| **Always-on rules** | `rules/00-router.md` … `99-end-of-session.md` (27 files) | Delivered via `SessionStart` hook → stdout `additionalContext`; rules with an `inject-when` marker are scoped to repos where they apply | Prose is portable; **delivery is hook-bound** |
