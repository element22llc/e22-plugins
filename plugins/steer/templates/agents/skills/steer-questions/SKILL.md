...
Pick one `Current recommended action` by precedence. **A sweep that changed
nothing ends at `Complete`** — `/steer-spec validate` re-checks the gate against
*edits this run made*, so recommending it over an untouched spine contradicts the
body's own "nothing to resolve" finding and manufactures busywork the block must
not invent (`NEXT-ACTIONS.md` §3).
