...
Bring a repo that never went through the E22 bootstrap — a "vibe-coded" app with
working code but no `/spec`, no `mise.toml`, no CI, no plugin install — into E22
...
template fork), use `/e22-standards:e22-init` instead; for a single feature
...
- **Ask, don't invent — humans decide product intent.** Product intent and
  ...
  intents. Run `/e22-standards:e22-questions` to resolve open questions.
...
- **Standards are not softened.** Adoption produces real spec, real tests, real
  Definition of Done — the same bar as any E22 repo. Gaps are recorded, not waived.
...
[`PROCEDURE.md`](${CLAUDE_PLUGIN_ROOT}/skills/e22-adopt/PROCEDURE.md)** — read the
phase you are on there before executing it.

1. **Confirm it's an adoption case** — no `/spec`, no `mise.toml`, not a template
   fork; detect the stack; branch `feat/e22-adopt`. → PROCEDURE Phase 1
...
12. **Hand off** — stamp `/spec/.version`, commit on `feat/e22-adopt`, propose the
