...
| `profiles/workspace/compose.yaml` | `compose.yaml` | **workspace** — **replaces** core compose. Declares **no services**: it composes each member's compose file with `include:` so one `mise run ws:dev` boots the product. Ships fully commented — `/steer:init` resolves one entry per member that has a compose file, or the file is deleted along with the `ws:docker:*`/`ws:dev` tasks if no member runs services. |
