...
type: tool_used
tool: Skill
input_match: 'steer:(init|setup)\b'
arm: both
weight: 3
---

The run must actually **enter** `init`, or the `setup` front door that detects a greenfield repo and hands off to it — asserted on the `Skill` tool call, not
on the prose.
