...
type: tool_used
tool: Skill
input_match: 'steer:(adopt|setup)\b'
arm: both
weight: 3
---

The run must actually **enter** `adopt`, or the `setup` front door that detects unspecified code and hands off to it — asserted on the `Skill` tool call, not
on the prose.
