...
| `/steer:explain` | Render a high-level, stakeholder-readable view of one feature spec as a shareable Claude Code Artifact — a private, hosted page on claude.ai — with a Markdown fallback where Artifacts are unavailable. A read-only, derived view: the `/spec` intent and tracker item stay canonical, on demand only, never auto-generated, and it writes nothing back into the spine. |
