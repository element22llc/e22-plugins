...
a greenfield bootstrap, not adoption of an existing app.

A passing response has to do **both** of these:

1. **Name the owning skill.** `/steer:init`, or its front door `/steer:setup`,
...
- does the **wrong** workflow's work instead - in particular `adopt`, which
