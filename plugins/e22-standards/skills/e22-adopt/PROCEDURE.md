# e22-adopt — detailed adoption procedure

Supporting runbook for the `/e22-standards:e22-adopt` skill. `SKILL.md` carries
...
There's no `/spec` spine, no `mise.toml`/E22 layout, and the repo was not forked
...
fresh adoption. (On an already-bootstrapped repo, `/e22-standards:e22-sync`
...
`/e22-standards:e22-spec-scaffold <id>` to create `intent.md` + `contract.md`).
...
E22 standards, the proposed Keep/Refactor/Rewrite/Reject disposition, and whether
...
**`/e22-standards:e22-adr`** — it stays `Proposed` until the named decider
...
the gap analysis against E22 standards — tests present? lockfiles committed and
...
cross-cutting → record it as a **`Proposed`** ADR (**`/e22-standards:e22-adr`**,
...
`e22-standards` plugin via the marketplace (dotfiles are stored without their
...
`/e22-standards:e22-issues bootstrap-labels` to create the
...
never from memory), so a later `/e22-standards:e22-sync` knows which structural
...
Commit on `feat/e22-adopt`. `PRODUCTIONIZATION.md` is the dev's productionization
...
- **To make selected gaps actionable** (GitHub tracker), run
  **`/e22-standards:e22-issues publish-adoption`** — it reconciles chosen gaps
