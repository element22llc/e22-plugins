...
| `policy/delivery.yml` | `policy/delivery.yml` | **Delivery policy** - the environments this repo has, what merging deploys, how production is approved, whether review apps exist, and what it reports to a human. Rule `52-deployment` follows this file rather than imposing a model; `/steer:protect` reads `production_gate`. Seeded with the org's AWS defaults; a repo that deploys elsewhere (or nowhere) edits it instead of arguing with a rule. |
