---
name: e22-work
description: "Execute a GitHub issue end-to-end from local Claude Code — read and validate the issue, claim it, create or reuse a branch, load linked specs, implement, test, update progress on the issue, open the PR, and transition lifecycle state. The execution counterpart to /e22-standards:e22-issues (which owns backlog management and never edits code). Routes all tracker-metadata I/O through /e22-standards:e22-tracker-sync; git and PR delivery follow the repo's commit/PR-autonomy rules. One issue per branch/PR by default."
...
backlog and never edits code; `/e22-standards:e22-work` reads an issue and delivers it.
...
2. **Route all tracker reads/writes through `/e22-standards:e22-tracker-sync`** (the gateway —
...
`/e22-standards:e22-tracker-sync find-or-create`. Create the separate issue when the work has
...
|---|---|---|
| Acceptance criteria not yet met | Blocking now (next transition) | Continue — `/e22-standards:e22-work resume #N` |
| Required validation failing | Blocking now | Fix failures, then `/e22-standards:e22-work finish #N` |
| Implemented, PR not opened | Blocking now (next transition) | `/e22-standards:e22-work finish #N` |
| PR open, in `validate`, awaiting review | Human decision required | A reviewer reviews the PR (no command) |
| PR merged but issue still `validate` (stale) | Blocking now | Reconcile to `done` — `/e22-standards:e22-work resume #N` |
| Issue `done` | Complete | Optional: start another ready issue — `/e22-standards:e22-work start #N`, else `No action is currently required.` |
