...
# Compound pre-rebrand token: the skill is the token AFTER `:e22-`, never
...
# Both WSL signals are stubbed per case, so the suite is hermetic on a real WSL2 host too.
PQ_PROC_PLAIN="${WORK}/pq-proc-plain"
printf 'Linux version 6.8.0-generic (gcc 13)\n' >"${PQ_PROC_PLAIN}"
PQ_PROC_WSL="${WORK}/pq-proc-wsl"
printf 'Linux version 5.15.0-microsoft-standard-WSL2 (gcc 11)\n' >"${PQ_PROC_WSL}"
pqscan "${PQ_OS}:${PQ_CORE}" "${PQR}" -u WSL_DISTRO_NAME "STEER_PROC_VERSION=${PQ_PROC_PLAIN}"
assert_eq "prereqs: os Linux (no WSL) -> linux" "$(pqstatus "${out}" os)" "linux"
pqscan "${PQ_OS}:${PQ_CORE}" "${PQR}" WSL_DISTRO_NAME=Ubuntu "STEER_PROC_VERSION=${PQ_PROC_PLAIN}"
assert_eq "prereqs: os Linux + WSL_DISTRO_NAME -> wsl2" "$(pqstatus "${out}" os)" "wsl2"
pqscan "${PQ_OS}:${PQ_CORE}" "${PQR}" -u WSL_DISTRO_NAME "STEER_PROC_VERSION=${PQ_PROC_WSL}"
assert_eq "prereqs: os Linux + microsoft /proc/version -> wsl2" "$(pqstatus "${out}" os)" "wsl2"
...
printf '%s' "${out}" | grep -q 'OpenSpec backend' &&
	bad "inject: repo without openspec/ must omit the OpenSpec spec-workflow rule" || ok

# openspec/ present -> 33-spec-workflow-openspec injected (has-openspec). The
# native rule 30 is UNMARKED by design and must still arrive: rule 33 overrides
# its paths in prose, and marking 30 would drop it from knowledge mode, where the
# injection banner promises spec-workflow still applies.
CRI_OS="$(new_repo cri_os)"
mkdir -p "${CRI_OS}/openspec/changes"
out="$(run_inject "$(session_json "${CRI_OS}" cri_os)")"
oq_grep "inject: openspec repo includes the OpenSpec spec-workflow rule" 'OpenSpec backend' "${out}"
oq_grep "inject: openspec rule names the propose command" '/opsx:propose' "${out}"
oq_grep "inject: openspec repo still receives the native spec-workflow rule" 'Brownfield' "${out}"
printf '%s' "${out}" | grep -q 'steer:inject-when' &&
	bad "inject: inject-when marker line must be stripped (openspec repo)" || ok

# A bare openspec/ directory is NOT proof of an OpenSpec spine (mirrors spine.sh
# refusing a bare spec/) -> the rule stays out.
CRI_OSBARE="$(new_repo cri_osbare)"
mkdir -p "${CRI_OSBARE}/openspec"
...
assert_ctx "comment-density: one third exactly is advisory" "${out}"
assert_no_block "comment-density: one third exactly does not block" "${out}"
# ...and the advisory tier stays once-per-file-per-session.
out="$(run_hook check-comment-density.sh "$(json_write "${RCD}" sCD3 src/third.ts 'x')")"
assert_empty "comment-density: advisory repeat silent" "${out}"
# (d2) below the 20% advisory floor -> silent.
{
	gen_lines 4 '// note %s'
	gen_lines 26 'const y%s = %s;'
} >"${RCD}/src/sparse.ts"
out="$(run_hook check-comment-density.sh "$(json_write "${RCD}" sCD12 src/sparse.ts 'x')")"
assert_empty "comment-density: under the advisory floor silent" "${out}"
...
out="$(run_hook check-comment-density.sh "$(json_write "${RCD}" sCD13 src/waived.ts 'x')")"
assert_empty "comment-density: allow-comments with a reason silences" "${out}"
{
	printf '// steer:allow-comments\n'
	gen_lines 16 '// step %s'
	gen_lines 8 'const z%s = %s;'
} >"${RCD}/src/bare.ts"
