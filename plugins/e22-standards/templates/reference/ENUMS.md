...
Human-readable documentation of every controlled vocabulary the E22 workflow
...
A materialized intent starts at **`draft`** (`/e22-standards:e22-issues
materialize` never approves). Only **`/e22-standards:e22-spec approve`** flips it
...
`/e22-standards:e22-spec approve` is blocked only by a `blocking` question with
`required_before: intent-approval`; later gates block their own transitions.

## `issue_kind` — the work shape (the `e22:kind` marker, not a label)

`feature · bug · task · finding · spec-question · spec-drift · audit-run`

Kind is the `e22:kind` marker + GitHub Issue Type, never a label. (See
`ISSUE-SCHEMA.md`.)

## `issue_state` — lifecycle state (the `e22:state` marker)
...
## `issue_source` — origin (the `e22:source` marker; the `source:*` label mirrors it)
