...
Presenting `/steer:spec` (or any command) as required. `Complete` must not name a mandatory continuation; the honest action is "no action required," and the convention explicitly permits that sentence.
