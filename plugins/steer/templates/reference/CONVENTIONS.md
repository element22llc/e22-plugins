...
- The scanner (`scripts/scan-version-pins.sh`) is read-only: it never executes
  repo content, follows no symlinks, skips `.git`, dependency trees and build
  output, and its diagnostics print only the matched token and its location,
...
- Lockfile-only diffs deserve the same review as code: an unexplained large
  ...
  change is the lockfile.
- **pnpm's `minimumReleaseAge` guard stays on.** pnpm 11 refuses packages
  published in the last 24 h by default. A plain `pnpm install` self-heals
...
- **Never set `container_name` or a fixed host port in `compose.yaml`.** Parallel
  Claude Code worktrees of one repo are isolated by `scripts/worktree-env.sh`
  (sourced by `mise.toml`): each linked worktree gets its own `POSTGRES_PORT`
...
An app-profile monorepo keeps its IaC tools in `infra/mise.toml`, not the root
toolchain, so the root `ci` workflow runs `tofu fmt` only when the *root* pins
tofu; otherwise that step skips with a notice rather than failing the required
check, and `/infra` is validated from its own toolchain.
...
- **Node.js → [pnpm](https://pnpm.io).** Use pnpm for installs and for workspaces
  ...
  every app and package shares one version and a bump happens in one place
  (exact versions stay pinned in `pnpm-lock.yaml`). pnpm gates dependency build
...
- **Changed-line regression → [`diff-cover`](https://github.com/Bachmann1234/diff_cover).**
  ...
  **no** global `--cov-fail-under` gate. The changed-line run also fires on a
  **push to `main`**: that is the solo-trunk Definition-of-Done floor, where no PR
  gated the lines. In pr-flow the PR already did, and re-gating the post-merge
  push would wedge `main`, so the push run is skipped there.
...
The scaffold `.gitattributes` pins `* text=auto eol=lf` (plus per-extension
pins) as the backstop: a Git for Windows checkout with `core.autocrlf=true`
would otherwise write CRLF, and a CRLF `scripts/*.sh` fails to *parse* while a
CRLF Docker entrypoint fails to run. Binaries are marked `binary` so they are
never normalized. Merge the file additively if one exists
(`scripts/scaffold_reconcile.py`).

<!-- Absorbed from the former always-on rule `20-layout`: a description of where
     directories live is reference material, not an imperative that has to be
     in context every session. Reach it via `/steer:reference conventions`. -->
