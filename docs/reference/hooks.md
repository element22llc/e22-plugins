...
flowchart TD
    subgraph SessionStart
      inject[inject-standards.sh<br/>injects always-on core rules/*.md<br/>capped at 10,000 chars]
...
| Hook | Matcher | Role |
| --- | --- | --- |
| `inject-standards.sh` | `startup\|resume\|clear\|compact` | Concatenates the **always-on core** `rules/*.md` (lexical order) into session context. It is **not** once-per-session: there is no guard, and `compact` can fire repeatedly within one session — deliberately, since a compaction can drop the injected rules and re-injecting is what puts them back. **It enforces Claude Code's 10,000-character cap on hook stdout**: the payload is assembled to a buffer, and if it would overrun, whole rules are dropped from the tail and an in-band `RULESET INCOMPLETE` notice names them (the untruncated list goes to stderr, which costs a session nothing). Without that guard the runtime silently persists the overflow to a file and hands the session a ~2 KB preview — the defect that made 35 of 36 rules undeliverable before the ruleset was split (see [Configuration & rules](configuration.md)). A rule carrying a first-line `<!-- steer:inject-when=… -->` marker is injected only when its scope applies, and the marker line is stripped; no current core rule uses one, but the mechanism and its scope predicates remain. In **knowledge-work mode** (a confidently non-code folder — the typical Cowork product-owner case) it skips every marked rule and emits a banner saying so (see [Knowledge-work mode](known-limitations.md)). Fail-soft: if its rules directory is missing it still emits a fallback banner (the hook always exits `0`, so the notice reaches the session) and records a self-fault for `/steer:report`. |
...
the unmarked always-on core. deferred repository rules scope themselves with `paths:`
frontmatter instead, so they are outside this predicate entirely.
