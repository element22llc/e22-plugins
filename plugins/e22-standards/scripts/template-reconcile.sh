...
[ -r "$existing" ] || { echo "template-reconcile: cannot read existing file: $existing" >&2; exit 3; }
[ -r "$bundled" ] || { echo "template-reconcile: cannot read bundled template: $bundled" >&2; exit 3; }
...
tmp_existing=$(mktemp) || { echo "template-reconcile: mktemp failed" >&2; exit 3; }
tmp_bundled=$(mktemp) || { rm -f "$tmp_existing"; echo "template-reconcile: mktemp failed" >&2; exit 3; }
