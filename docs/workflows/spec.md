...
!!! info "When to use"
    ...
    compare against the code later via `/steer:drift`.
...
flowchart LR
    ...
    work -. compare later .-> drift["/steer:drift"]
    spec -. is the baseline for .-> drift
```

The spec is the in-repo source of truth that `/steer:drift` later compares the
built code against.
