...
flowchart TD
    ...
    subgraph PreToolUse
      ...
      ascii[check-ascii-writes.sh<br/>ASCII in code and values]
...
| `check-ascii-writes.sh` | `Write\|Edit\|MultiEdit\|NotebookEdit` | Enforces **[rule 85](configuration.md)'s "ASCII in code and values"** at the keystroke instead of leaving it to always-on prose: a typographic character introduced into a **value position** is denied, and the reason names each offender with its codepoint and the ASCII substitute (`em dash U+2014 -> -`), so the rewrite is one step. The failure it prevents is concrete — an AWS resource name or `description` carrying an em dash is rejected by the API's validator, at deploy time rather than at the write. A deny rather than a notice because the check is deterministic, a `PostToolUse` notice cannot unwrite the file, and a mechanical fixup is unsafe (rewriting a curly quote inside a string literal can break its quoting). **Comment-aware by design:** rule 85 permits these characters in prose, and the bundled scaffold relies on that (every `mise.toml`/`compose.yaml`/CI script opens with a typographic header comment), so comments are stripped before the scan and a header comment is never flagged. Every stripping heuristic over-strips deliberately — a `//` counts as a comment only when it does not follow a `:`, so a URL survives — because a missed character merely leaves the rule in force while a false deny is what gets a gate switched off. Scoped to `implementation`/`operations` files with a known comment syntax plus `.hcl`; documentation, the `/spec` spine, lockfiles, generated output and unrecognized types are untouched, as is the plugin's own source repo. Only the **introduced** text is inspected (`content` / `new_string` / every `edits[].new_string` / `new_source`), so an edit that *removes* an em dash is never denied. The matched set is exactly rule 85's — dashes, curly quotes, ellipsis, bullet, arrows, non-breaking and thin spaces — so accented Latin letters, guillemets, CJK and emoji never trip it and non-English copy is safe. A deliberate character is declared with `steer:allow-typographic` in the same content (mirroring `steer:allow-pin`). Ported to the Copilot CLI as an `ask`. **Known gap:** `Bash` heredoc writes are not inspected, the same documented bypass `check-version-pins.sh` carries. |
