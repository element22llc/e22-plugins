...
| `profiles/workspace/mise.toml` | `mise.toml` (repo root) | **workspace** — **replaces** core mise. Drops pnpm/biome (no code here), keeps the agent-runtime baseline + `convert:doc` (PO documents land at the spine host), and adds the `ws:*` member tasks + a `dev` that boots every member. The `[settings] monorepo_root` / `[monorepo]` blocks ship **commented out** — uncomment once members are cloned, listing `config_roots` explicitly (auto-discovery is deprecated upstream) and setting `lockfile = false` so members stay buildable standalone. |
| `profiles/workspace/compose.yaml` | `compose.yaml` | **workspace** — **replaces** core compose. Declares **no services**: it composes each member's compose file with `include:` so one `mise run dev` boots the product. Ships fully commented — `/steer:init` resolves one entry per member that has a compose file, or the file is deleted along with the `docker:*`/`dev` tasks if no member runs services. |
