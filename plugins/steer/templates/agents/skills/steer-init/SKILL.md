...
via `/steer-setup protect`. The `feat/*` + PR default below is for repos with a reviewer;
...
**walk the migration ledger, then stamp `spec/.version`**. The ledger comes
first: a fork predates the version-keyed entries, and stamping before applying
them makes `/steer-sync` skip them for good. The stamp is what makes the fork
`managed` rather than `foreign`.
...
2. **Instantiate the bundled scaffold - core plus the profile's extras.** The
   ...
   rules are in
   [`SCAFFOLD.md`](SCAFFOLD.md) - read it
...
3. **Interview to fill the spine.** Populate `vision.md`, `users.md`,
   ...
   [`INTERVIEW.md`](INTERVIEW.md) - **read it
...
7. **Hand off.** Seed the first `/spec/history/` entry - one file,
   ...
   **Stamp the spine version:** write
   ...
   `https://github.com/element22llc/e22-plugins/blob/main/plugins/steer/.claude-plugin/plugin.json` - never from memory) so a
   ...
   **Solo trunk mode:** commit the bootstrap **directly to `main`** with no PR
   ...
   (`/steer-setup protect`) - there is no bootstrap PR. Otherwise (a repo with a reviewer):
...
| Solo trunk mode: bootstrap committed to `main`, no PR | Recommended | Spec or build the first feature; graduate via `/steer-setup protect` when the MVP works - or, if the repo will stay single-dev on trunk, record that with `/steer-setup protect waive` |
