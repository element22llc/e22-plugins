...
!!! warning "Hooks are a Claude Code lifecycle feature - don't assume they ran"
    ...
    **advisory nudges** that let the write proceed (the two dimensions of
    ...
    `check-version-pins` and `check-ascii-writes` issue a hard `deny`, and the trunk-push gate in
...
flowchart TD
    ...
    subgraph PreToolUse
      ...
      ascii[check-ascii-writes.sh<br/>ASCII everywhere]
...
| `check-ascii-writes.sh` | `Write\|Edit\|MultiEdit\|NotebookEdit` | Enforces **[rule 85](configuration.md)'s "ASCII everywhere"** at the keystroke instead of leaving it to always-on prose: a write that introduces a typographic character - em/en dash, curly quote, ellipsis, bullet, arrow, non-breaking or thin space - is denied, and the reason names each offender with its codepoint and the ASCII substitute (`em dash U+2014 -> -`), so the rewrite is one step. Two reasons it is a hard rule: strict validators reject them (an AWS resource name or `description` carrying an em dash fails at deploy time, not at the write), and they read as machine-written. A deny rather than a notice because the check is deterministic, a `PostToolUse` notice cannot unwrite the file, and a mechanical fixup is unsafe (rewriting a curly quote inside a string literal can break its quoting). **Prose is not exempt.** An earlier version stripped comments and scanned only value positions; the scan is now raw, so a comment, a spec, a Markdown doc and a config value are all held to the same rule. Scope is every authored text class (`implementation`, `operations`, `documentation`, `spec`) plus `.hcl`; lockfiles, generated output and unrecognized types stay out, as does the plugin's own source repo - which is covered instead by the committed-state gate `scripts/check-ascii.sh` (`mise run check-ascii`, also a pre-commit hook), sharing the same character table. Only the **introduced** text is inspected (`content` / `new_string` / every `edits[].new_string` / `new_source`), so an edit that *removes* an em dash is never denied. The matched set is exactly rule 85's, so accented Latin letters, guillemets, CJK and emoji never trip it and non-English copy is safe. A deliberate character is declared with `steer:allow-typographic` in the same content (mirroring `steer:allow-pin`). Ported to the Copilot CLI as an `ask`. **Known gap:** `Bash` heredoc writes are not inspected, the same documented bypass `check-version-pins.sh` carries. |
...
| `check-comment-density.sh` | `Write\|Edit\|MultiEdit` | Makes the **Code comments** rule (`08-code-comments.md`) visible at the moment it is broken: reads the just-written file from disk (files under 20 non-blank lines are exempt) and reports the ratio in **two tiers** - from **a fifth** of its non-blank lines an advisory `additionalContext`, **once per file per session**; from **a third** a `decision: "block"` carrying the same text as its `reason`, which **re-fires** while the file stays over, **capped at three** per file per session - the cap resets once the file drops back into the advisory band, since it exists to stop nagging on a file that cannot be trimmed, not on one that was. On `PostToolUse` a block does not undo the write - Claude Code places the `reason` beside the tool result - so this is a louder notice, not a gate; the PR review remains the wall. A file-level **`steer:allow-comments <reason>`** in any comment silences both tiers; the reason is required, so a bare marker suppresses nothing (unlike the per-line `# steer:allow-pin`, density is a property of the whole file, and there is no legacy alias). Counts only a line's leading marker - `#` for shell/Python/TOML/YAML/Dockerfile/HCL, `//` and block-comment bodies for the C-family and TypeScript/JavaScript, `--` for SQL/Lua - so a `#` inside a string never counts; Markdown, JSON, HTML and unknown types are skipped, and so is `dependabot.yml`, whose per-stack blocks are commented-out code by necessity. A **shebang** and a **linter/type-checker directive** (`# shellcheck`, `# noqa`, `# type:`, `# ruff:`, `// eslint-disable`, `// biome-ignore`, `// @ts-expect-error`, ...) are machine-read instructions rather than prose, so they count toward the line total but not as comments. Markers live in `TMPDIR`; the hook always exits `0` and is exempt in the plugin's own source repo. **Ported to the Copilot CLI** under the PascalCase `PostToolUse` key, where it emits a top-level `additionalContext` (the CLI documents only the flat key and drops Claude's nested `hookSpecificOutput` form); `postToolUse` has no `decision` field there, so the block tier prefixes its text with `BLOCKED:` rather than being dropped on that surface. |
...
- editor writes pass `tool_input.file_path` / `.notebook_path`
  (`check-write-nudges`, `check-version-pins`, `check-ascii-writes`, `format-on-write`,
...
`hooks/copilot-hooks.json` for it (the ruleset injector under `sessionStart`, the
three `PreToolUse` gates, and the `PostToolUse` comment-density notice). Copilot
Chat in VS Code instead runs this
