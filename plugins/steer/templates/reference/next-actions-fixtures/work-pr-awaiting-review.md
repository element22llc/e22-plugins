...
`/steer:work finish #123` (already in `validate`) or `Complete`. The issue is not `done` until the PR merges, so the workflow is not `Complete`; the open PR is the current human gate.
