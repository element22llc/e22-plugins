...
| Spec | `/spec` | Dev or PO (via `/steer:build`) writes; PO approves intent; dev approves the PR | Durable |
...
├── sources/                    # Versioned home for recurring PO source documents, maintained by /steer:spec intake
│   └── README.md
├── reference/                  # Durable one-off source/research material feeding the spec (inventories, vendor metadata, DDL dumps, discovery docs) - created on demand by /steer:work tidy, no shipped README
├── features/
│   └── [feature-id]/
...
`<!-- steer:question-id=Q-001 -->` (see [`ISSUE-SCHEMA.md`](ISSUE-SCHEMA.md)); the
...
1. **Specs are written with Claude's help - by a dev, or by a PO via `/steer:build`.** The PO approves intent; a dev approves the PR before merge. POs are not expected to write specs from scratch.
...
**`/steer:sync` is the dedicated driver** for an already-bootstrapped repo;
`/steer:adopt` and `/steer:build` apply the same ledger inline on a resume so a paused
