...
**When to use.** Reached via /steer-work issues - not a direct entry point.
...
- **`capture`, `status` and `materialize` use the core ops only** (`find`,
  `get`, `create`, `update-state`, `claim`, `comment`, `link-delivery`,
  `close`). The modes that reach past them - `decompose`/`epic` (types, parent
  links), `triage`'s Priority floor, `board`'s ranking read, `reconcile`'s label
  and type normalization - read `OPERATIONS-GITHUB.md` for the one op they are
  about to perform, and degrade where the capability is absent.
...
table in `https://github.com/element22llc/e22-plugins/blob/main/plugins/steer/templates/reference/ISSUE-WORKFLOW.md`; the issue
...
`modes/`. **Read the one file for the mode
you are running; do not read the others.**

| Mode | What it does | Procedure |
|---|---|---|
| `brainstorm #N` | Product discovery against an issue, no spec written | [`delegating.md`](modes/delegating.md) |
| `materialize #N` | Approved intent -> a draft `intent.md` via `/steer-spec` | [`delegating.md`](modes/delegating.md) |
| `publish-audit` | File `/steer-audit` findings as parent + children | [`delegating.md`](modes/delegating.md) |
| `publish-drift` | File `/steer-audit spec` drift as decision checklists | [`delegating.md`](modes/delegating.md) |
| `publish-adoption` | Reconcile `PRODUCTIONIZATION.md` gaps into findings | [`delegating.md`](modes/delegating.md) |
| `publish-findings` | File code-review / security-review findings | [`delegating.md`](modes/delegating.md) |
| `capture` | Open an issue from the conversation or a design source | [`backlog.md`](modes/backlog.md) |
| `triage [#N\|--all]` | Dedup, label, fill field gaps, route | [`backlog.md`](modes/backlog.md) |
| `board [--all]` | Read-only ranked, relationship-aware backlog view | [`backlog.md`](modes/backlog.md) |
| `bootstrap-labels` | Idempotently reconcile the label taxonomy | [`backlog.md`](modes/backlog.md) |
| `decompose #N` | Feature -> implementation sub-issues (contract-gated) | [`hierarchy.md`](modes/hierarchy.md) |
| `epic` | The tier above features: group features under one parent | [`hierarchy.md`](modes/hierarchy.md) |
| `status [#N\|feature-id]` | Unified read-only issue + spec + readiness view | [`hierarchy.md`](modes/hierarchy.md) |
| `reconcile` | Verify issue <-> spec pointers and lifecycle consistency | [`reconcile.md`](modes/reconcile.md) |
