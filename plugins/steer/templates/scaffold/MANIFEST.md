...
| `claude/rules/steer-*.md` | `.claude/rules/steer-*.md` | **The delivered half of the org ruleset — 30 files.** Claude Code caps a hook's stdout at 10,000 characters, so only the six always-on core rules fit in steer's SessionStart injection; these carry the rest. Each has `paths:` frontmatter and is injected automatically when Claude reads a file it governs (`spec/**` for the spec workflow, test globs for testing, `**` for the action-scoped rules like commit autonomy that no path predicts). They are **plugin-managed**: never hand-edit a `steer-*.md` here — change the rule in the plugin and re-run `/steer:sync`, which reconciles them. A consumer's own `.claude/rules/*.md` files sit alongside untouched, since the `steer-` prefix scopes reconciliation. Removing one silently drops that standard from the repo. |
