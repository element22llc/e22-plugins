...
// (`/presentation/sdlc/`) clicking "next" pushes
// `/presentation/sdlc/#/presentation/sdlc/2`, which matches no route (slides
// register at `/:no`) and lands on Slidev's in-app NotFound page.
//
...
// 404.html as an SPA fallback — never a nested `presentation/sdlc/404.html` —
      ...
      transform(code, id) {
        ...
        if (!code.includes(BUGGY)) {
          ...
          throw new Error(
            ...
            + 'routing and update or remove presentation/sdlc/vite.config.ts.',
          )
        }
