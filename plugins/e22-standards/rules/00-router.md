...
- Spec ↔ code coupling rules → opened by **`/e22-standards:e22-spec-scaffold`**.
...
- Tracker is GitHub Issues and you just want the low-level gateway? → run **`/e22-standards:e22-tracker-sync`** — the tracker-metadata API (search/get/find-or-create/update/comment/label/set-type/transition/link/close, pull/push for drift) behind `/e22-standards:e22-issues` and `/e22-standards:e22-work` (MCP-first, `gh` fallback; moves tracker metadata, not the spec, and never git/PR delivery).
