...
a standards-conformance sweep, not the current-state report status gives.

A passing response has to do **both** of these:

1. **Name the owning skill.** `/steer:audit` appears somewhere in the response -
...
- does the **wrong** workflow's work instead - in particular `status`, which
