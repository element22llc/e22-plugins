...
| `policy/branch-protection.yml` | `policy/branch-protection.yml` | **Branch-protection policy** (the GitHub-side PR gate `main` must enforce). Read by `/steer:protect`, which diffs it against the repo's live settings and applies the gap on confirmation. Seeded from the plugin default; the product may tighten it, or select the plugin-defined `solo` profile (`profile: solo`) for a one-person repo. |
| `policy/delivery.yml` | `policy/delivery.yml` | **Delivery policy** - the environments this repo has, what merging deploys, how production is approved, whether review apps exist, and what it reports to a human. Rule `45-delivery` § Deployment & environments follows this file rather than imposing a model; `/steer:protect` reads `production_gate`. Seeded with the org's AWS defaults; a repo that deploys elsewhere (or nowhere) edits it instead of arguing with a rule. |
