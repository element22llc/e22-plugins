...
a branch *ahead* of `main` is `[ok]` and no pending fragments is `[info]`, both
...
- **Every fix touching `plugins/steer/` gets its own changelog fragment** - a new
  file under `.changes/unreleased/`, named
  `<kind>-<YYYYMMDD>-<HHMM>-<slug>.yaml`. Never edit `CHANGELOG.md`: it is
  generated by `changie merge` at release, so an edit here is both reverted later
  and failed by `check_changelog.py` now.

  **Do not expect a gate to catch a missing fragment on this branch.**
  `check_changelog.py --base` asks whether *some* fragment was added across
  `origin/main...HEAD` - not whether this round added one, and not whether it
  describes this round. So round 1's fragment satisfies it permanently and it can
  never fire again for rounds 2+. `check_docs_impact.py --base` is coarse in
  exactly the same way ("any docs change clears the gate", per its own
  docstring). What actually catches a missing entry is the **next round's
  dimension 1** - one full round later. The per-round re-run in **L3.f** is what
  closes that window; the fragment is your job either way.

- **Revising an earlier round's entry is safe now.** Each round's entry is its own
  file, so rewording or deleting one is an ordinary edit to a path nothing else
  touches - no merge driver resolves it, and a concurrent PR cannot resurrect
  retracted wording. (This replaced a real hazard: when entries shared one
  `CHANGELOG.md` region under `merge=union`, union kept *both* sides of a
  conflicting hunk, so a retraction could silently come back with no conflict
  marker and no gate complaint.)
...
- Would a fragment this round added survive dimension 1 - does it
